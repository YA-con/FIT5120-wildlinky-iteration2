...
for i, species in enumerate(species_ids):
    ...
    trace = go.Scattermapbox(
        ...
        marker=dict(
            ...
            line=dict(width=1, color='black')  # Adds border
        ),
        name=species,
        customdata=df_s[['year', 'region', 'site_desc']],
...
years = list(range(2010, 2021))
colors = px.colors.qualitative.Plotly
symbols = ['circle', 'square', 'triangle', 'cross', 'x']
color_map = {species: colors[i % len(colors)] for i, species in enumerate(species_ids)}
...
for i, species in enumerate(species_ids):
    df_s = df[df['name'] == species]
    symbol_choice = symbols[i % len(symbols)]
    print(f"🔍 {species}: using symbol '{symbol_choice}'")  # Debug output
    trace = go.Scattermapbox(
        lat=df_s['lat'],
        lon=df_s['long'],
        mode='markers',
        marker=dict(
            size=10,
            color=color_map[species],
            symbol=symbol_choice  # Different shape per species
        ),
        name=species,
        customdata=df_s[['year', 'region', 'site_desc']],
        hovertemplate="<b>Year:</b> %{customdata[0]}<br><b>Site:</b> %{customdata[2]}<extra></extra>"
